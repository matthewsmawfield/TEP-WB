...
First published: 18 August 2025 · Last updated: 29 April 2026
...
- A1. Two-metric structure on a single manifold. Gravity is described by a Lorentzian metric $g_{\mu\nu}$; matter fields, clocks, and rulers couple to a causal (matter) metric $\tilde{g}_{\mu\nu}$. The metrics are related by a disformal map
                $$\tilde{g}_{\mu\nu} = A^2(\phi) g_{\mu\nu} + B(\phi) \nabla_\mu\phi \nabla_\nu\phi,$$
                with a universal conformal factor $A(\phi) = \exp(\beta \phi/M_{\text{Pl}})$ and a small disformal function $B(\phi)$ consistent with multi-messenger constraints today.
...
|--------|-----------------|----------|-----------|
| Conformal $A(\phi)$ | Clock rates, proper time rescaling | Universal ($\beta$) | Cassini PPN-$\gamma$, redshift tests |
...
|----------------|------------|-----------------|-------------------|--------------------------|
| **TEP (This work)** | $g_{\mu\nu}$, $\phi$ | Universal to $\tilde{g}_{\mu\nu} = A^2 g_{\mu\nu} + B \partial_\mu\phi \partial_\nu\phi$ | No, if $B \neq 0$ | Synchronization Holonomy $H_{\text{resid}}$ |
...
This quantity is invariant under admissible synchronization-protocol shifts $\tilde{\sigma} \rightarrow \tilde{\sigma} + d\chi$ because $\oint_C d\chi = 0$.
...
Proof sketch. Parameterize the path coordinate $s \in [0,L]$; write $t_\to = \int ds A(\phi_0 + s \partial_\parallel\phi)/c$ and $t_\leftarrow$ similarly along the reverse. Linear terms in $\partial_\parallel\phi$ cancel exactly; see Appendix A for full derivation.
...
Define the synchronization one-form $\tilde{\sigma} = -\tilde{N}^{-1} \tilde{N}_i dx^i$. For a closed spatial loop $C$, the raw loop integral is

$$H = \oint_C \tilde{\sigma},$$

but the physical TEP observable is the GR-subtracted residual

$$H_{\text{resid}} = \oint_C (\tilde{\sigma} - \sigma_{\text{GR}}) = \iint_\Sigma (d\tilde{\sigma} - d\sigma_{\text{GR}}).$$

In GR with stationary spacetimes, the raw holonomy reproduces the Sagnac/gravito-magnetic effect; these known contributions are subtracted using geodesy and ephemerides. The residual holonomy beyond GR is
...
where $\kappa_X$ is an observable response coefficient for channel $X$, not the microscopic conformal coupling $\beta$ and not a PPN coupling. The locally active PPN coupling is suppressed by the environmental/source screening factor $\mathcal S_\Sigma(\mathcal E)$ and should not be confused with channel response coefficients $\kappa_X$.
...
PPN recovery in the screened limit (DEF framework). In the Damour–Esposito-Farèse parametrization, the Jordan-frame metric for a static, spherically symmetric source is
...
where $\alpha_{\rm eff} \equiv d(\ln A)/d\phi|_{\phi_{\rm source}}$ is the effective coupling at the source. When screening suppresses the Temporal Shear, $\nabla\phi \to 0$ and the field settles to $\phi_{\rm source} \approx {\rm const}$, giving $\alpha_{\rm eff} \to 0$. The PPN parameters then reduce exactly to GR values:
$$\gamma - 1 = -2\alpha_{\rm eff}^2 \to 0, \quad \beta - 1 = \frac{\alpha_{\rm eff}^2}{2} \frac{d\alpha_{\rm eff}}{d\phi} \to 0.$$
Thus $\gamma = \beta = 1$ in the screened limit, independent of the cosmological value $\alpha_0$.
...
Observable. $H_{\text{resid}} = \oint \tilde{\sigma} - \text{(GR model)}$. Forecast magnitude $O(10^{-18}$–$10^{-16})$ fraction per loop time (0.1–1 s), limited by disformal and time-varying $A(\phi)$ effects. Null in GR by design.
...
Static $\phi$ gradients do not generate first-order one-way anisotropy. An explicit derivation (Appendix A) shows cancellation, correcting prior misinterpretations.

Holonomy invariant and not a synchronization artifact. The observable $H_{\text{resid}}$ is constructed from physical proper-time measurements and is the integral of a curvature two-form $\Delta(d\tilde{\sigma})$ beyond GR; protocol changes shift $\tilde{\sigma}$ by exact differentials, which integrate to zero over closed loops.
...
Cosmology claims are hypotheses to be tested. The outline shows how modest $\phi$ dynamics can ease $H_0/S_8$ while respecting early-universe bounds; quantitative MCMC fits are required and planned.
...
| $A(\phi)$ | Conformal coupling factor, $\exp(\beta \phi/M_{\text{Pl}})$ | 2 |
| $B(\phi)$ | Disformal coupling function | 2 |
| TEP | Temporal Equivalence Principle | 1, 2 |
| $\tau$ | Proper time defined by $\tilde{g}_{\mu\nu}$ | 2 |
| $H_{\text{resid}}$ | residual synchronization holonomy, $H_{\text{resid}}(C) \equiv \oint_C (\tilde{\sigma} - \sigma_{\text{GR}})$ | 3, 6 |
...
| $\beta$ | Dimensionless conformal coupling parameter | 2 |
...
| $\tilde{u}_\mu$ | Four-velocity of an observer/clock | 3.2 |
