...
def _numpy_bin_fit_rs(sep, vals, bins, bin_centers):
    ...
    try:
        popt, _ = curve_fit(
            tep_screening_model_fixed_alpha,
            bin_centers[valid], meds[valid], sigma=sems[valid],
...
def permutation_test_delta_rs(sub_df, group_a_mask, group_b_mask, value_column, bins, rng, n_perm=PERMUTATION_ITERATIONS):
    ...
    rs_obs_a = _numpy_bin_fit_rs(sep_np[a_in_union], val_np[a_in_union], bins, bin_centers)
    rs_obs_b = _numpy_bin_fit_rs(sep_np[~a_in_union], val_np[~a_in_union], bins, bin_centers)
    observed_delta = (rs_obs_a - rs_obs_b) if (np.isfinite(rs_obs_a) and np.isfinite(rs_obs_b)) else np.nan
...
def perform_environment_test():
    ...
    print_status("Joint profile-likelihood fit: shared α_sat profiled out, separate R_s...", "PROCESS")
    ...
    res_h1 = scipy_minimize(joint_chi2_separate_rs,
    ...
    res_h0 = scipy_minimize(joint_chi2_shared_rs,
                             x0=[0.35, 4000.0],
    ...
    p_lrt = float(1.0 - chi2_dist.cdf(delta_chi2_joint, df=1)) if delta_chi2_joint > 0 else 1.0

    print_status(f"Joint H1 (separate R_s): α = {alpha_joint:.3f}, R_s(low Z) = {rs_low_joint:.0f}, R_s(high Z) = {rs_high_joint:.0f}, χ² = {chi2_h1:.1f}", "RESULT")
    print_status(f"Joint H0 (shared R_s):   α = {alpha_null:.3f}, R_s = {rs_null:.0f}, χ² = {chi2_h0:.1f}", "RESULT")
    print_status(f"Likelihood ratio test: Δχ² = {delta_chi2_joint:.1f}, p = {p_lrt:.2e} (1 dof)", "RESULT")
    ...
    for _ in range(200):
        ...
        try:
            br = scipy_minimize(_boot_obj, x0=[alpha_joint, rs_low_joint, rs_high_joint],
                                 bounds=[(0.01, 0.8), (100.0, 50000.0), (100.0, 50000.0)],
                                 method='L-BFGS-B')
    ...
    if len(boot_rs_low_j) > 10:
        ...
        boot_ordering_frac = float(np.mean(np.array(boot_rs_low_j) > np.array(boot_rs_high_j)))
        print_status(f"Joint bootstrap: α = {alpha_joint:.3f} ± {alpha_joint_err:.3f}", "INFO")
        print_status(f"Joint bootstrap: R_s(low Z) = {rs_low_joint:.0f} ± {rs_low_joint_err:.0f}", "INFO")
        print_status(f"Joint bootstrap: R_s(high Z) = {rs_high_joint:.0f} ± {rs_high_joint_err:.0f}", "INFO")
        print_status(f"Ordering preserved in {boot_ordering_frac*100:.1f}% of bootstrap resamples", "RESULT")
    else:
    ...
    print_status("Sensitivity sweep: varying fixed α_sat to test ordering robustness...", "PROCESS")
    ...
    for alpha_test in alpha_sweep_values:
        ...
        try:
            popt_low, _ = curve_fit(_model_alpha_test, prof_low['sep_AU'], prof_low['v_tilde_norm'],
                                     sigma=prof_low['sem'], p0=[2000], bounds=([100], [50000]), maxfev=10000)
            popt_high, _ = curve_fit(_model_alpha_test, prof_high['sep_AU'], prof_high['v_tilde_norm'],
                                      sigma=prof_high['sem'], p0=[2000], bounds=([100], [50000]), maxfev=10000)
            ...
            print_status(f"  α = {alpha_test:.2f}: R_s(low Z) = {rs_low_t:.0f}, R_s(high Z) = {rs_high_t:.0f} → ordering preserved: {ordering}", "INFO")
        except (RuntimeError, ValueError):
            alpha_sweep_results.append({
                'alpha_fixed': alpha_test, 'rs_low_z': np.nan, 'rs_high_z': np.nan, 'ordering_preserved': False,
            })
            print_status(f"  α = {alpha_test:.2f}: fit failed", "WARNING")

    n_preserved = sum(r['ordering_preserved'] for r in alpha_sweep_results)
    print_status(f"Ordering preserved in {n_preserved}/{len(alpha_sweep_values)} fixed-α values tested", "RESULT")
    ...
    print_status(f"Median |Z|: midplane = {Z_low_med*1000:.0f} pc, high-|Z| = {Z_high_med*1000:.0f} pc", "INFO")
    print_status(f"ρ_baryonic: midplane = {rho_low_med:.4f}, high-|Z| = {rho_high_med:.4f} M☉/pc³", "INFO")
    ...
    print_status(f"Chameleon n=1 prediction: R_s ratio = {predicted_ratio_n1:.3f}", "RESULT")
    print_status(f"Observed joint-fit ratio: {obs_ratio_joint:.3f} ± {obs_ratio_joint_err:.3f}", "RESULT")
    print_status(f"Predicted R_s(high-|Z|) = {predicted_rs_high_n1:.0f} AU (observed: {rs_high_joint:.0f} ± {rs_high_joint_err:.0f})", "RESULT")
        ...
        if log_rho != 0 and log_obs != 0:
            ...
            if len(boot_rs_low_j) > 10:
                ...
                if len(n_boot) > 10:
                    ...
                    print_status(f"Bootstrap: n = {n_inferred_med:.2f} ± {n_inferred_err:.2f} (median ± std)", "INFO")
                else:
    ...
    for n_test in [0.5, 1.0, 1.5, 2.0]:
        ...
        print_status(f"  n={n_test:.1f}: predicted ratio = {pred:.3f}, R_s(high) = {pred_rs:.0f} AU", "INFO")
    ...
    R_s_full_m = 2646.0 * 1.496e11  # 2646 AU in metres
    ...
    print_status(f"Full-sample median |Z| = {Z_full_med*1000:.0f} pc, ρ_baryonic = {rho_full_med:.4f} M☉/pc³", "INFO")
    ...
    ax_cham.plot(Z_grid_kpc * 1000, rs_grid_n1, 'k-', linewidth=1.2, label='Chameleon $n=1$ prediction')
    # Data points from joint fit
    ax_cham.errorbar([Z_low_med * 1000], [rs_low_joint], yerr=[rs_low_joint_err],
                      fmt='s', color='#4682B4', markersize=7, capsize=3, label='Midplane (joint fit)')
    ax_cham.errorbar([Z_high_med * 1000], [rs_high_joint], yerr=[rs_high_joint_err],
                      fmt='D', color='#6A5ACD', markersize=7, capsize=3, label='High-$|Z|$ (joint fit)')
    ...
    fig_cham.savefig(figures_dir / "chameleon_prediction.png", dpi=600)
    ...
    print_status("MLR Sensitivity Test: checking environmental ordering under alternative mass corrections...", "PROCESS")
    ...
    if 'c_ref' in df.columns and 'Mg2' in df.columns and 'bp_rp2' in df.columns:
        coeffs_ridge = np.polyfit(
            df.loc[(np.abs(df['Z_gc']) < 0.1) & (df['Mg1'] > 4) & (df['Mg1'] < 10) & df['bp_rp1'].notna(), 'Mg1'],
            df.loc[(np.abs(df['Z_gc']) < 0.1) & (df['Mg1'] > 4) & (df['Mg1'] < 10) & df['bp_rp1'].notna(), 'bp_rp1'],
            3
        )
    ...
    def fit_env_ordering(vt_arr, label):
        ...
        df_tmp = df_tmp.replace([np.inf, -np.inf], np.nan).dropna(subset=['v_tilde_alt'])
        ...
        try:
            popt_l, _ = curve_fit(tep_screening_model_fixed_alpha, pl['sep_AU'], pl['v_tilde_norm'],
                                   sigma=pl['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_l = float(popt_l[0])
        except (RuntimeError, ValueError):
            rs_l = np.nan
        try:
            popt_h, _ = curve_fit(tep_screening_model_fixed_alpha, ph['sep_AU'], ph['v_tilde_norm'],
                                   sigma=ph['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_h = float(popt_h[0])
        except (RuntimeError, ValueError):
            rs_h = np.nan
        ordering = "YES" if (np.isfinite(rs_l) and np.isfinite(rs_h) and rs_l > rs_h) else "NO"
        print_status(f"  {label}: R_s(low Z) = {rs_l:.0f}, R_s(high Z) = {rs_h:.0f} → ordering preserved: {ordering}", "RESULT")
    ...
    beta1_quad = 1.5
    beta2_quad = 0.5
    m1_quad = np.where(has_color1, m1_base * (1 + beta1_quad * dc1 + beta2_quad * dc1**2), m1_base)
    m2_quad = np.where(has_color2, m2_base * (1 + beta1_quad * delta_c2 + beta2_quad * delta_c2**2), m2_base)
    vt_quad = recompute_vtilde(m1_quad, m2_quad)
    rs_l_q, rs_h_q, ord_q = fit_env_ordering(vt_quad, "Quadratic MLR (β₁=1.5, β₂=0.5)")
    mlr_sensitivity_results.append({'mlr_model': 'quadratic_b1_1.5_b2_0.5', 'rs_low_z': rs_l_q, 'rs_high_z': rs_h_q, 'ordering': ord_q})
    ...
    rs_l_none, rs_h_none, ord_none = fit_env_ordering(vt_none, "No MLR correction (β=0)")
    mlr_sensitivity_results.append({'mlr_model': 'no_correction_beta_0', 'rs_low_z': rs_l_none, 'rs_high_z': rs_h_none, 'ordering': ord_none})
    ...
    mlr_sensitivity_results.append({'mlr_model': 'linear_beta_2.0', 'rs_low_z': rs_l_s, 'rs_high_z': rs_h_s, 'ordering': ord_s})
    ...
    mlr_sensitivity_results.append({'mlr_model': 'linear_beta_1.0', 'rs_low_z': rs_l_sh, 'rs_high_z': rs_h_sh, 'ordering': ord_sh})

    n_preserved_mlr = sum(1 for r in mlr_sensitivity_results if r['ordering'] == 'YES')
    print_status(f"Environmental ordering preserved in {n_preserved_mlr}/{len(mlr_sensitivity_results)} MLR variants tested", "RESULT")
    ...
    for drop_idx in range(n_bins_env):
        ...
        pl_lobo = prof_low.iloc[[i for i in range(len(prof_low)) if i != drop_idx]] if drop_idx < len(prof_low) else prof_low.copy()
        ph_lobo = prof_high.iloc[[i for i in range(len(prof_high)) if i != drop_idx]] if drop_idx < len(prof_high) else prof_high.copy()

        try:
            popt_l, _ = curve_fit(tep_screening_model_fixed_alpha, pl_lobo['sep_AU'], pl_lobo['v_tilde_norm'],
                                   sigma=pl_lobo['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_l_lobo = float(popt_l[0])
        except (RuntimeError, ValueError):
            rs_l_lobo = np.nan
        try:
            popt_h, _ = curve_fit(tep_screening_model_fixed_alpha, ph_lobo['sep_AU'], ph_lobo['v_tilde_norm'],
                                   sigma=ph_lobo['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_h_lobo = float(popt_h[0])
        except (RuntimeError, ValueError):
            rs_h_lobo = np.nan

        ordering_lobo = "YES" if (np.isfinite(rs_l_lobo) and np.isfinite(rs_h_lobo) and rs_l_lobo > rs_h_lobo) else "NO"
        bin_center = 10**(0.5*(np.log10(bins[drop_idx]) + np.log10(bins[drop_idx+1])))
        lobo_results.append({
    ...
    print_status(f"Leave-one-bin-out: ordering preserved in {n_lobo_preserved}/{len(lobo_results)} bin drops", "RESULT")
    for r in lobo_results:
        if r['ordering'] == 'NO':
            print_status(f"  Ordering lost when dropping bin at {r['dropped_bin_center_AU']:.0f} AU", "WARNING")
    ...
    print_status(f"  Distance-matched range: {d_lo:.0f}–{d_hi:.0f} pc ({n_dm_low} low-Z, {n_dm_high} high-Z)", "INFO")
    ...
    if n_dm_low > 1000 and n_dm_high > 1000:
        ...
        prof_dm_low = get_binned_profile(df_dm, np.abs(df_dm['Z_gc']) < 0.1, base_dm_low)
        prof_dm_high = get_binned_profile(df_dm, np.abs(df_dm['Z_gc']) > 0.15, base_dm_high)
        try:
            popt_dml, _ = curve_fit(tep_screening_model_fixed_alpha, prof_dm_low['sep_AU'], prof_dm_low['v_tilde_norm'],
                                     sigma=prof_dm_low['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_dml = float(popt_dml[0])
        except (RuntimeError, ValueError):
            rs_dml = np.nan
        try:
            popt_dmh, _ = curve_fit(tep_screening_model_fixed_alpha, prof_dm_high['sep_AU'], prof_dm_high['v_tilde_norm'],
                                     sigma=prof_dm_high['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_dmh = float(popt_dmh[0])
        except (RuntimeError, ValueError):
            rs_dmh = np.nan
        ord_dm = "YES" if (np.isfinite(rs_dml) and np.isfinite(rs_dmh) and rs_dml > rs_dmh) else "NO"
        print_status(f"  Distance-matched: R_s(low Z)={rs_dml:.0f}, R_s(high Z)={rs_dmh:.0f} → ordering: {ord_dm}", "RESULT")
        matched_results.append({'control': 'distance_matched', 'rs_low_z': rs_dml, 'rs_high_z': rs_dmh, 'ordering': ord_dm})
    ...
    if 'bp_rp1' in df.columns:
        ...
        print_status(f"  Color-matched range: BP-RP {c_lo:.2f}–{c_hi:.2f} ({n_cm_low} low-Z, {n_cm_high} high-Z)", "INFO")
        ...
        if n_cm_low > 1000 and n_cm_high > 1000:
            ...
            prof_cm_low = get_binned_profile(df_cm, np.abs(df_cm['Z_gc']) < 0.1, base_cm_low)
            prof_cm_high = get_binned_profile(df_cm, np.abs(df_cm['Z_gc']) > 0.15, base_cm_high)
            try:
                popt_cml, _ = curve_fit(tep_screening_model_fixed_alpha, prof_cm_low['sep_AU'], prof_cm_low['v_tilde_norm'],
                                         sigma=prof_cm_low['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
                rs_cml = float(popt_cml[0])
            except (RuntimeError, ValueError):
                rs_cml = np.nan
            try:
                popt_cmh, _ = curve_fit(tep_screening_model_fixed_alpha, prof_cm_high['sep_AU'], prof_cm_high['v_tilde_norm'],
                                         sigma=prof_cm_high['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
                rs_cmh = float(popt_cmh[0])
            except (RuntimeError, ValueError):
                rs_cmh = np.nan
            ord_cm = "YES" if (np.isfinite(rs_cml) and np.isfinite(rs_cmh) and rs_cml > rs_cmh) else "NO"
            print_status(f"  Color-matched: R_s(low Z)={rs_cml:.0f}, R_s(high Z)={rs_cmh:.0f} → ordering: {ord_cm}", "RESULT")
            matched_results.append({'control': 'color_matched', 'rs_low_z': rs_cml, 'rs_high_z': rs_cmh, 'ordering': ord_cm})
    ...
    print_status(f"  Strict RUWE<1.1: {n_strict_low} low-Z, {n_strict_high} high-Z (from {mask_strict_ruwe.sum()} total)", "INFO")
    ...
    if n_strict_low > 1000 and n_strict_high > 1000:
        ...
        prof_sr_low = get_binned_profile(df_strict, np.abs(df_strict['Z_gc']) < 0.1, base_sr_low)
        prof_sr_high = get_binned_profile(df_strict, np.abs(df_strict['Z_gc']) > 0.15, base_sr_high)
        try:
            popt_srl, _ = curve_fit(tep_screening_model_fixed_alpha, prof_sr_low['sep_AU'], prof_sr_low['v_tilde_norm'],
                                     sigma=prof_sr_low['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_srl = float(popt_srl[0])
        except (RuntimeError, ValueError):
            rs_srl = np.nan
        try:
            popt_srh, _ = curve_fit(tep_screening_model_fixed_alpha, prof_sr_high['sep_AU'], prof_sr_high['v_tilde_norm'],
                                     sigma=prof_sr_high['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_srh = float(popt_srh[0])
        except (RuntimeError, ValueError):
            rs_srh = np.nan
        ord_sr = "YES" if (np.isfinite(rs_srl) and np.isfinite(rs_srh) and rs_srl > rs_srh) else "NO"
        print_status(f"  Strict RUWE<1.1: R_s(low Z)={rs_srl:.0f}, R_s(high Z)={rs_srh:.0f} → ordering: {ord_sr}", "RESULT")
        matched_results.append({'control': 'strict_ruwe_1.1', 'rs_low_z': rs_srl, 'rs_high_z': rs_srh, 'ordering': ord_sr})
    ...
    for n_alt_bins, label_bins in [(12, '12-bin'), (18, '18-bin')]:
        ...
        prof_alt_low = get_binned_profile.__wrapped__(df, mask_low_z, base_low_z) if hasattr(get_binned_profile, '__wrapped__') else None
        ...
        def _alt_profile(sub_df, mask, base_val, b=bins_alt):
            ...
            grouped = sub.groupby('bin', observed=True)['v_tilde_norm']
            return pd.DataFrame({
                'sep_AU': 10**(0.5 * (np.log10(b[:-1]) + np.log10(b[1:]))),
                'v_tilde_norm': grouped.median(),
                'sem': 1.253 * grouped.std() / np.sqrt(grouped.count())
            }).dropna()
        ...
        try:
            popt_al, _ = curve_fit(tep_screening_model_fixed_alpha, pal['sep_AU'], pal['v_tilde_norm'],
                                    sigma=pal['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_al = float(popt_al[0])
        except (RuntimeError, ValueError):
            rs_al = np.nan
        try:
            popt_ah, _ = curve_fit(tep_screening_model_fixed_alpha, pah['sep_AU'], pah['v_tilde_norm'],
                                    sigma=pah['sem'], p0=[2000], bounds=([100],[50000]), maxfev=10000)
            rs_ah = float(popt_ah[0])
        except (RuntimeError, ValueError):
            rs_ah = np.nan
        ord_alt = "YES" if (np.isfinite(rs_al) and np.isfinite(rs_ah) and rs_al > rs_ah) else "NO"
        print_status(f"  {label_bins}: R_s(low Z)={rs_al:.0f}, R_s(high Z)={rs_ah:.0f} → ordering: {ord_alt}", "RESULT")
        matched_results.append({'control': f'bins_{n_alt_bins}', 'rs_low_z': rs_al, 'rs_high_z': rs_ah, 'ordering': ord_alt})

    n_matched_preserved = sum(1 for r in matched_results if r['ordering'] == 'YES')
    print_status(f"Matched/robustness controls: ordering preserved in {n_matched_preserved}/{len(matched_results)} tests", "RESULT")
    ...
    cham_profile.to_csv(out_dir / "chameleon_rs_profile.csv", index=False)

    if mlr_sensitivity_results:
        pd.DataFrame(mlr_sensitivity_results).to_csv(out_dir / "mlr_sensitivity.csv", index=False)

    if lobo_results:
        pd.DataFrame(lobo_results).to_csv(out_dir / "leave_one_bin_out.csv", index=False)

    if matched_results:
        pd.DataFrame(matched_results).to_csv(out_dir / "matched_subsample_controls.csv", index=False)

    if alpha_sweep_results:
        pd.DataFrame(alpha_sweep_results).to_csv(out_dir / "environment_alpha_sweep.csv", index=False)
